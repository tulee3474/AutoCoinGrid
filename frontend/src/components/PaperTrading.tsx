                  ...
                  <tr key={pos.id} className="border-b border-border/40 hover:bg-white/3">
                    <td className="py-2 pr-3">
                      <span
                        ...
                        className={`text-[10px] px-1 py-0.5 rounded mr-1 ${
                          (pos.side ?? 'SHORT') === 'LONG'
                    ...
                    <td className="py-2 pr-3 text-down num" title={pos.liquidationPrice ? `추정 청산가 $${pos.liquidationPrice.toPrecision(5)} (마스터 조회 키 설정 시 정확도 향상)` : undefined}>
                      ${pos.stopLossPrice.toPrecision(4)}
                      {pos.liquidationPrice != null && (() => {
                        ...
                        return (
                          <div className="text-[10px] text-gray-600">청산(추정) ${pos.liquidationPrice.toPrecision(4)} (+{liqPct.toFixed(1)}%)</div>
                        );
                      })()}
                    </td>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              <Link to="/logs" className="text-xs text-accent hover:underline">전체 로그 보기 →</Link>
              <button
